...
X_LABEL <- ""
Y_LABEL <- "Total" 
GRAPH_TITLE <-"Ferramentas Utilizadas pelos Participantes"
X_AXIS_INCREMENT <- 10
IMG_DIR <-'./img/'
FILE_NAME <- 'grafico_melhorias_fgrm_ferramentas_utilizadas'
...
#Plotando o gráfico
ggplot(data=df_data, aes(x=reorder(df_data$ferramenta_utilizada, df_data$total),
                                    y=df_data$total, fill=df_data$ferramenta_utilizada
                        )
       ) +
