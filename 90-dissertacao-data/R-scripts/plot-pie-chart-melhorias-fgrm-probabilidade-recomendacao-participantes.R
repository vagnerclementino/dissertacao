...
CSV_FILE <-'./data/201702051251_melhorias_fgrm_probabilidade_recomendacao.csv'
FILE_SEPARATOR <- ","
WIDTH_BAR <- .3
LEGEND_NAME <- "Virou Extensão"
X_LABEL <- ""
Y_LABEL <- "Total" 
GRAPH_TITLE <-"Probabilidade de Recomendação da FGRM Utilizada"
X_AXIS_INCREMENT <- 10
IMG_DIR <-'./img/'
FILE_NAME <- 'grafico_melhorias_fgrm_probabilidade_recomentacao'
...
bp <- barplot(total_estudos,
              ...
              axes=F,
              main=GRAPH_TITLE,
              col = gray.colors(length(df_data$probabilidade_recomendacao),
                                start = 1,
                                end = 0),
              las = 2
...
labs <- df_data$probabilidade_recomendacao
