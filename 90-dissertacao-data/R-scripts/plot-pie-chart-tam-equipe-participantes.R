...
CSV_FILE <-'./data/201702040922_tamanho_equipe_participante.csv'
FILE_SEPARATOR <- ","
WIDTH_BAR <- .3
LEGEND_NAME <- "Virou Extensão"
X_LABEL <- ""
Y_LABEL <- "Frequência" 
GRAPH_TITLE <-"Perfil dos Participantes: Tamanho da Equipe"
X_AXIS_INCREMENT <- 10
IMG_DIR <-'./img/'
FILE_NAME <- 'grafico_escolha_ferramentas_tamanho_equipe'
...
#Plotando o gráfico
ggplot(data=df_data, aes(x=reorder(df_data$tamanho_equipe, df_data$frequencia),
                                    y=df_data$frequencia, fill=df_data$tamanho_equipe
                        )
       ) +
