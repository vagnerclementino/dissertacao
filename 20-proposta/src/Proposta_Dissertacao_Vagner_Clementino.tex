...
Esse controle é geralmente realizado por Sistemas de Controle de Demandas (SCD)- Issue Tracking Systems,  que ajudam os desenvolvedores na correção de forma individual ou colaborativa, dentre outros de defeitos (bugs) e desenvolvimento de novas funcionalidades. Verifica-se na literatura diversos sinônimos para os Sistemas de Controle de Demanda (Sistema de Controle de
...
Grande parte da literatura em manutenção de software tratam de técnicas e metodologias tradicionais da Engenharia de Software. Contudo, é cada vez mais latente a adoção de metodologias ágeis em projetos de sucesso \cite{Serrador2015}, mesmo em áreas não relativas à tecnologia da informação. Neste contexto, verifica-se uma tendência de que os departamentos dedicados à manutenção de software estão bem cientes da metodologia ágil e estão começando a experimentá-las em suas atividades \cite{Heeager2015}. Apesar de desenvolvimento e manutenção de software serem atividades distintas \todo{Esta é uma visão tradicional. Devemos aceitá-la?}, esta última adaptar característica da primeira visando a melhoria do seu desempenho. Dentre as características do métodos ágeis passíveis de serem utilizadas em tarefas de manutenção podemos citar processo de desenvolvimento interativo, maior envolvimento do cliente, comunicação face a face, testes frequentes, entre outras.

\todo[inline]{END: MANUTENÇÃO ÁGIL}


\todo[inline]{BEGIN: JUSTIFICATIVA}
Da mesma forma que o ocorreu no desenvolvimento de software, é possível verificar uma crescente adoção de técnicas da metodologia ágil na manutenção de software \cite{Soltan2016,Devulapally2015, Heeager2015}. Neste contexto, é natural que ferramentas que dão suporte à manutenção de software, tal como as FSPS, tenham que evoluir a esta nova forma de trabalhar. Mesmo em um ambiente tradicional de  desenvolvimento e manutenção de software, verifica-se a necessidade de adequação das FSPS. Tal siutação ocorre tendo em vista que a maioria desses sistemas são projetados em torno do termo "demanda"
