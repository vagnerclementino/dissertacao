...
CSV_FILE <-'./data/201702051241_melhorias_fgrm_nivel_satisfacao.csv'
FILE_SEPARATOR <- ","
WIDTH_BAR <- .3
LEGEND_NAME <- "Virou Extensão"
X_LABEL <- ""
Y_LABEL <- "Total" 
GRAPH_TITLE <-"Nível de Satisfação com as Funcionalidades"
X_AXIS_INCREMENT <- 10
IMG_DIR <-'./img/'
FILE_NAME <- 'grafico_melhorias_fgrm_nivel_satisfacao'
...
#Plotando o gráfico
ggplot(data=df_data, aes(x=reorder(df_data$nivel_satisfacao, df_data$total),
                                    y=df_data$total, fill=df_data$nivel_satisfacao
                        )
       ) +
