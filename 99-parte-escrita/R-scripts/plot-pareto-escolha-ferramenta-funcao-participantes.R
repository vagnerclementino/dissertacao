  ...
  CSV_FILE <-'./data/201702040922_escolha_ferramenta_funcao_participantes.csv'
  FILE_SEPARATOR <- ","
  GRAPH_TITLE <-"Perfil dos Participantes: Função Desempenhada"
  IMG_DIR <-'./img/'
  FILE_NAME <- 'grafico_escolha_ferramentas_funcao_participantes'
  ...
  total_estudos <- df_data$frequencia
  #names(total_estudos) <- df_data$funcao
  total_estudos_sort <- sort(df_data$frequencia, decreasing=TRUE)
  ...
  bp <- barplot(total_estudos,
                ...
                col = gray.colors(df_data$funcao,
                                  start = 1,
                                  end = 0),
                las = 2
