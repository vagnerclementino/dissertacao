...
CSV_FILE <-'./data/201701102354_total_por_papel.csv'
FILE_SEPARATOR <- ","
WIDTH_BAR <- .3
LEGEND_NAME <- "Papel"
X_LABEL <- "Papel"
Y_LABEL <- "Total de artigos" 
GRAPH_TITLE <-"Total de Artigos por Papel na Manutenção de Software"
X_AXIS_INCREMENT <- 2
IMG_DIR <-'./img/'
FILE_NAME <- 'grafico_papel_por_artigo'
...
#Plotando o gráfico
ggplot(data=df_data, aes(x=reorder(df_data$papel, df_data$total),
                                    y=df_data$total, fill=df_data$papel
                        )
       ) + 
