...
CSV_FILE <-'./data/201702050910_melhorias_fgrm_localizacao_participantes.csv'
FILE_SEPARATOR <- ","
WIDTH_BAR <- .3
LEGEND_NAME <- "Virou Extensão"
X_LABEL <- ""
Y_LABEL <- "Total" 
GRAPH_TITLE <-"Perfil dos Participantes: Localização Geográfica"
X_AXIS_INCREMENT <- 10
IMG_DIR <-'./img/'
FILE_NAME <- 'grafico_melhorias_fgrm_localizacao_geografica'
...
#Plotando o gráfico
ggplot(data=df_data, aes(x=reorder(df_data$localizacao, df_data$total),
                         y=df_data$total, fill=df_data$localizacao
)
) +
