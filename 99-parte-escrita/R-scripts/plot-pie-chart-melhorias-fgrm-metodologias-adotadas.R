...
CSV_FILE <-'./data/201702051635_metodologias_adotadas.csv'
FILE_SEPARATOR <- ","
WIDTH_BAR <- .3
LEGEND_NAME <- "Virou Extensão"
X_LABEL <- ""
Y_LABEL <- "Total" 
GRAPH_TITLE <-"Processo de Desenvolvimento Adotado"
X_AXIS_INCREMENT <- 10
IMG_DIR <-'./img/'
FILE_NAME <- 'grafico_melhorias_fgrm_metodologia_adotadas'
...
#Plotando o gráfico
ggplot(data=df_data, aes(x=reorder(df_data$metodologia, df_data$total),
                                    y=df_data$total, fill=df_data$metodologia
                        )
       ) +
