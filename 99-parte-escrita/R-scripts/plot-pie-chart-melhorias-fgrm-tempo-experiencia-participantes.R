...
CSV_FILE <-'./data/201702051045_melhorias_fgrm_tempo_experiencia.csv'
FILE_SEPARATOR <- ","
WIDTH_BAR <- .3
...
X_AXIS_INCREMENT <- 10
IMG_DIR <-'./img/'
FILE_NAME <- 'grafico_melhorias_fgrm_tempo_experiencia'
...
#Plotando o gráfico
ggplot(data=df_data, aes(x=reorder(df_data$tempo_experiencia, df_data$total),
                                    y=df_data$total, fill=df_data$tempo_experiencia
                        )
       ) +
