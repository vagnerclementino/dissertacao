...
\begin{itemize}
	...
	\item \textbf{$RQ_2$}: \textit{Quais papeis envolvidos no processo de manutenção de software a extensão visa dar suporte?}
	\item \textbf{$RQ_3$}: \textit{Qual técnica foi utilizada para desenvolver a extensão?}
	\item \textbf{$RQ_4$}: \textit{Quais as FGRM estão sendo estendidas?}
