...
%Revisão: dom set 24 09:44:15 -03 2017
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
76\% dos participantes disseram que recomendariam as FGRMs que utilizava para
...
funções propostas nas FGRMs que utilizavam. A partir da análise deste resultado
é possível inferir que o grupo de desenvolvedores que participaram do
...
As sugestões propostas neste capítulo não estão vinculadas exclusivamente às
...
já existentes. Cabe-nos ressaltar que não houve a intenção de apresentar um
conjunto exaustivo de melhorias.
...
sugestões foram estruturadas como seções deste capítulo, em que apresentamos a
...
\sugestao{01}{As FGRMs devem fornecer realimentação (\textit{feedback})
    relacionada à qualidade do texto relatado.}
...
\sugestao{02}{As FGRMs devem fornecer busca pelo código fonte contido no
...
a facilidade de acesso ao código fonte pode propiciar na redução do tempo de
conclusão da tarefa de manutenção e no aumento da qualidade das alterações
...
\sugestao{03}{As FGRMs devem permitir um ranqueamento das RMs de acordo com a
...
usuários cujos relatos frequentemente envolvem questões de segurança do
...
\sugestao{04}{As FGRMs devem fornecer atalhos para filtros
...
\sugestao{05}{As FGRMs devem fornecer suporte a Processos de Integração
    Contínua.}
...
Nestas situações não faz sentido que responsável por fechar a RM engatilhe um
...
\sugestao{06}{As FGRMs devem dar suporte além da especificação de texto
    simples.}
...
\sugestao{07}{As FGRMs devem fornecer uma classificação automática em termos
    da urgência da RM.}

\paragraph{Justificativa:}\label{par:justificativa_s07}
...
software~\cite{svensson2005introducing}. Nestes ambientes, um dos problemas
...
\paragraph{Exemplo de Utilização:}\label{par:exemplo_de_utilização_s07}
...
\sugestao{08}{As FGRMs devem suportar tarefas compartilhadas, permitindo o
...
recomendado. Tivemos que nos conformar com uma amostra composta por
...
do questionário está Apêndice~\ref{ch:app-form-sug-melhorias}.

\subsection{Processo de Aplicação}\label{ssub:processo_de_aplicação}

O questionário foi encaminhado via correio eletrônico. O endereço foi coletado
diretamente do projeto hospedado no Github. O pedido de participação foi
enviado para um total de 121 contribuidores no período de 28/04/2017 a
01/05/2017. A mensagem enviada pode ser visualizada no
...
de Software que utilizam o levantamento por questionário como método de coleta
de dados~\cite{fan2010factors}. Nas próximas seções apresentamos o perfil dos
participantes, a relevância das sugestões propostas e o grau de facilidade de
implementação das mesmas.

\subsection{Perfil dos Participantes}\label{sub:sug_melhorias_resultados_perfil_dos_participantes}
...
desenvolvedores (53\%). O segundo grupo de função com maior frequência são
...
Engenheiros e Arquitetos de Software. Sobre o tempo de experiência, cerca de 13
participantes têm entre dez e vinte anos de experiência.  A maior parte
desempenha suas atividades em equipes com 2 a 5 pessoas ou com mais do 10
membros. Quando questionamos sobre qual o tipo de atividade desempenhada pelo
participante, cerca de 25 deles trabalham com desenvolvimento ou manutenção de
software.
...
Concordo Fortemente). A exceção foi para a Sugestão \#3 \@-\@ \textit{As FGRMs
    devem permitir um ranqueamento das RMs de acordo com a reputação do
...
pela reputação do Reportador. Por outro lado, as Sugestões \#6 \@-\@ \textit{As
    FGRMs devem dar suporte além da especificação de texto simples}  e \#8
\@-\@ \textit{As FGRMs devem suportar tarefas compartilhadas, permitindo o
...
\begin{figure}[htpb]
    ...
    \caption{Avaliação sobre a  implementação das sugestões propostas.}\label{fig:plot_likert_avaliacao_implementacao_melhorias}
\end{figure}
...
Contudo, para comprovarmos este tipo de hipótese seria necessário conduzir um
...
\caption{Ordenamento das sugestões pelo grau de dificuldade.}\label{tab:ranking_implementacao_sug_melhorias}
\end{table}

\section{Discussão}\label{sec:sug_melhoria_discussao}
...
uma visão mais positiva, como por exemplo ``Concordo'' ou ``Concordo
...
aceita. Este tipo de sugestão tem como principal objetivo aumentar o poder de
expressão do relato, tal como a possibilidade do destaque da sintaxe do código
fonte. Conforme pode ser observado na
...
demonstra, com base na amostra utilizada, que deveria ser estendida para outras
ferramentas.

O suporte a tarefas compartilhadas (sugestão \#8) também foi muito bem aceito.
...
FGRMs (sugestões \#6, \#4, e \#2) foram consideradas como mais ``fáceis'' de
...
Para avaliarmos as sugestões propostas nos conformamos em empregar uma amostra
que se caracteriza como de conveniência, apresentando os vícios desse tipo de
amostragem. Apesar da taxa de resposta estar dentro da faixa observada na
literatura, o total de participantes não nos permite extrapolar os resultados
para todos os contextos em que as FGRMs estão inseridas.  Adicionalmente, os
critérios utilizados para seleção, como por exemplo, seis meses de
desenvolvimento ou ter no mínimo duzentas revisões, não nos permite afirmar que
foram escolhidos os projetos mais representativos para o nosso público-alvo. A
utilização de apenas projetos públicos hospedados no Github pode ter causado
algum tipo de direcionamento, por exemplo, o foco em projetos de código aberto.
...
item. Por outro lado, se o texto for escrito de forma ``concisa'' corremos o
risco de ficar vago, o que pode ter impacto nas respostas.  Uma boa opção
talvez fosse uma descrição concisa associada com uma explicação detalhada. A
opção por apenas uma descrição concisa pode ter feito a pesquisa ficar
superficial.
