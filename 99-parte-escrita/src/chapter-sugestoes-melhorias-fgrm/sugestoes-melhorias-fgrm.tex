...
satisfeitos com a ferramenta utilizada, contudo, \textit{não conhecem ou não têm
    acesso ao potencial de funções que este tipo de software poderia oferecer}.
...
(plugins) para as FGRMs com base no que foi proposto de modo a aplicar essas
melhorias sugeridas em sua rotina de trabalho.
...
implementar todas elas, especialmente por entendermos que a análise desta
...
de uma funcionalidade existente. Cada sugestão foi estruturada como uma seção
...
ativação (stack track), dificultam mais o andamento do projeto do que RMs
...
informações necessárias à sua solução. Neste caso, o programador seria
diretamente beneficiado já que teria a sua disposição um relato mais completo.
Não obstante, um trabalho adicional seria dado ao reportador que em algumas
...
avalia o texto corresponde ao atributo \textit{relato} da RM gerada e apresenta
ao responsável algumas dicas de como a informação fornecida poderia ser
...
para uma FGRM com estas características. Os detalhes estão descritos no
...
comparação com a uma busca estruturada, presente em grande parte das FGRMs, por
...
Seção~\ref{sub:fluxo_de_trabalho_requisicao_mudanca}. Desta forma, seria
...
ou que permitam realce da sintaxe do código poderiam ajudar ao Reportador a
...
para a criação de uma RM, que no contexto da plataforma Github corresponde ao
...
interrompidas o que pode ocasionar frustração e desmotivação com relação ao
...
estudo que serão discutidas na Seção~\ref{sec:sug_melhoria_ameacas}. Os critérios
...
\begin{itemize}
    ...
    \item Os projetos obtidos devem estar os 50 mais populares que atendam aos
...
resultados em que o reenvio pode ser um dos fatores que aumentam a taxa de
...
obrigatório é possível que o número de repostas seja menor. O nível de
participação foi similar ao observado em outros estudo em Engenharia de Software
que utilizam o levantamento por questionário (survey) como método de coleta de
...
afirmar que escolhemos os projetos mais representativos para o nosso
público-alvo.
