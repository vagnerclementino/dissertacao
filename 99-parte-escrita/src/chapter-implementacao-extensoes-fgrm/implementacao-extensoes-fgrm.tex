...
aspectos de implementação da Sugestão \#1\@-\@\textit{As FGRMs devem fornecer
    realimentação (feedback) relacionado à qualidade do texto relatado} na
plataforma Github. A escolha da Sugestão \#1 e da plataforma Github envolve
mais aspectos conveniência do que algum tipo de decisão formal.
...
apresentamos com mais detalhe o desenho e o funcionamento da extensão proposta.
...
desenvolvimento da extensão que exigem a utilização da linguagem. Após
...
ótica. Tomando como base os testes executado, verificamos que, em média, o
tempo que a extensão leva para analisar uma RM, não causa impacto
no seu processo de solução. Contudo, seria importante replicar o mesmo testes
de duração em um ambiente controlado de modo a remover qualquer tipo de
enviesamento.
...
corpo da \textit{RM}.Não verificamos estudos que comprovem esta hipótese nem os
nossos resultados permitem comprová-la. Por outro lado, um fator a destacar é
que mais da metade das \textit{RMs} analisadas apresentaram um texto com baixa
legibilidade. Esta situação já foi verificada em estudos
...
Neste sentido pode ser interessante ao projeto de software disciplinar os
