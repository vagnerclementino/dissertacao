...
\begin{figure}[htpb] \centering
	...
	\caption{Processo de Manutenção de
		Software adaptado da norma IEEE~1219~\cite{IEEE1999-Std1219}.}
...
\begin{figure}[htpb]
    ...
    \caption{Processo de Manutenção de Software adaptado da norma
        ISO/IEC~14764~\cite{institute2006norma}.}
...
da organização com as necessidades do time de desenvolvimento. Por outro lado,
a adoção de um modelo de manutenção de software usando práticas da Programação
Extrema (\textit{Extreme Programming \@-\@ XP}) apresentou como resultado
melhorias no aprendizado e produtividade da equipe por meio do aumento da
moral, encorajamento e confiança entre os
...
\paragraph{Submetida (\textit{Submit}):}
\label{par:submetida)}
...
\paragraph{Em Revisão (\textit{Review}):}
...
\paragraph{Em Análise (\textit{Analysis}):}
...
\paragraph{Compromissada (\textit{Commit}):}
\label{par:commit}
...
\paragraph{Em Implementação (\textit{Implement}):}
...
\paragraph{Em Verificação (\textit{Verification}):}
...
\paragraph{Fechada (\textit{Closed}):}
\label{par:fechada}
...
\paragraph{Rejeitada (\textit{Decline}):}
\label{par:rejeitada}
...
replicado. Na segunda, apresentamos uma lista previamente definida de
...
tinha em média de 10 anos de trabalho na areá. O formulário foi modificado com
...
Anexo~\ref{ch:app-form-selecao-ferramentas}. Como o público-alvo do
questionário poderia incluir desenvolvedores de diferentes nacionalidades foi
criada uma versão em inglês\footnote{A versão inglês do formulário está
    disponínvel em
...
os respectivos documentos que foram utilizados. Para aquelas sistemas que
...
partir dos dados utilizados~\cite{just2008towards}. A técnica envolve a
...
outros~\cite{maiden1996acre} sugere que a técnica dos Cartões de Classificação
seja uma das mais úteis para aquisição de conhecimento de dados.
...
O processo de trabalho relacionado com a manutenção de software pode consumir ou
