...
\begin{figure}[htpb]
	\centering
	\includegraphics[width=0.5\linewidth]{./chapter-manutencao-software-visao-geral/img/diagrama-caso-uso-reportador.pdf}
...
Deve planejar a fila de RMs e atribuí-las para o desenvolver mais apto. A
decisão pode considerar a carga de trabalho existente para cada desenvolvedor.

\paragraph{Desenvolvedor:}
Responsável por realizar as ações que irão solucionar a RM\@.

\paragraph{Analista de Qualidade:}
Tem por responsabilidade avaliar se uma RM solucionada por um Desenvolvedor afim
...
\begin{figure}[htpb]
	\centering
	\includegraphics[width=0.5\linewidth]{./chapter-manutencao-software-visao-geral/img/rm-exemplo.pdf}
...
\begin{itemize}
   ...
   \item \textit{Comunicação clara das RMs entre as partes
       ...
       equipe de manutenção. O resultado por avaliar de maneira distinta uma RM
...
\textit{Fechada (Closed)} onde é considerada como solucionada. Neste caminho o
...
\begin{figure}[htpb]
	\centering
	\includegraphics[width=0.5\linewidth]{./chapter-manutencao-software-visao-geral/img/diagrama-estado-rm-codigo-aberto.pdf}
...
as desenvolvedores~\cite{anvik2005coping}. Em alguns casos esta solução não é
...
tipo de manutenção deverá ser realizada. Geralmente este tipo de classificação é
feita com base no texto que corresponde ao relato da RM\@. A diversidade de
...
novo membro da equipe nem sempre é possível ou desejável. A premissa é que o
mentor poderia ser mais útil fazendo tarefas mais
importantes~\cite{malheiros2012source}.  Por exemplo, quando um novo
desenvolvedor entra na equipe seria interessante que ele resolvesse as RMs que
...
projeto é fundamental. No entanto, encontrar um defeito apropriado ao nível de
...
gerenciamento de projetos, comunicação, discussão e revisão de código. Neste
trabalho utilizaremos o termo \texttt{Ferramentas de Gerenciamento de
    Requisições de Mudança} (FGRM) ao nos referir a este tipo de
softwares.

As RMs são controladas por uma FGRM na forma de um fluxo de trabalho de modo a
identificar, descrever e controlar a situação de cada RM\@. Em geral, os
...
fato que as FGRMs não apenas ajudam as organizações a gerenciar, atribuir,
...
de atribuição de uma RM está presente na maior parte das FGRMs, todavia, segunda
...
A primeira etapa consistiu da definição das ferramentas que seriam utilizadas
...
aberto e código proprietário, que tenham utilizado alguma FGRM\@. Segundo o
nosso entendimento não houve a necessidade de analisar todos as ferramentas
disponíveis. Um conjunto mínimo, avaliado como representativo pelos
participantes do levantamento, poderia fornecer as informações necessárias para
subsidiar este estudo. A representatividade no escopo do levantamento
...
de software não estarem entre no público-alvo desta dissertação.
...
sistematizados por meio de técnica denominada \textit{Cartões de
    Classificação~-~Sorting Cards}. Trata-se de uma técnica de elicitação de
conhecimento de baixo custo e com foco no usuário. Ela é utilizada na área de
arquitetura da informação para criar modelos mentais e derivar taxonomias a
partir dos dados utilizados~\cite{just2008towards}. Ela envolve a categorização
de um conjunto de cartões em grupos distintos de acordo com um critério
previamente definido~\cite{mcgee2009software}. O estudo de Maiden e
...
FGRM Bugzilla e o cartão que foi gerado para a mesma.

\begin{figure}[htpb]
	\centering
	\includegraphics[width=0.5\linewidth]{./chapter-estudo-funcionalidades-fgrm/img/documentacao_bugzilla.png}
...
Nesta seção inicialmente mostramos os resultados inicialmente apresentando o
...
possui entre 3 e 10 anos (60\%). Na segunda posição temos os participantes com
10~-~20 anos de experiência (17\%). Com relação ao tamanho da equipe de que os
participantes fazem parte, verificamos uma prevalência de equipes de médio
(mais do que 10 membro) e pequenas (2 a 5 membros) porte. Por sua vez, estas
equipes estão predominantemente em empresas privadas de software, que
representou 37 participantes. Com relação ao local de trabalho verificamos
ainda que o segundo posto em número de par\-ti\-ci\-pan\-tes ficou para
empresas que pertencem ao setor governamental, do qual tivemos 11
participantes. O restante é composto por um profissional que se dedica a
projetos de software livre e um estudante.
...
Este perfil profissional tem o conhecimento necessário para nos ajudar no
...
inicial, contudo, apareceram neste resultado final. Isso decorre de atribuirmos
...
deste tipo de hi\-e\-rar\-qui-\-a. Como o nosso objetivo é derivar tópicos a
...
de modo a construir um espectro de funcionalidades para as FGRM que pode ser
...
Tabela~\ref{tab:freq_categorias_cartoes} em que é possível verificar ainda a
frequência que cada uma das categorias apareceu no conjunto de cartões coletado.
...
uma RM utilizando o texto relatado na RM, mediante filtros personalizáveis ou
...
suportem RMs de forma agregada permitindo o gerenciamento em massa da informação
armazenada. Este tópico contempla as funcionalidades que se dedicam ao
armazenamento e consistência das informações contidas na FGRM\@. As ferramentas
...
desenvolvimento e manutenção de software. Nela estão incluídos funcionalidade
...
Neste tópico estão as características das FGRM que ajudam no desenvolvimento
...
As FGRMs possuem funcionalidades que permitem a realização do papel ao qual este
tipo de software se propõe. Não obstante, devido à sua crescente importância,
seria necessário que este tipo de ferramenta incorpore funções e comportamentos
que ajudem no processo de desenvolvimento e manutenção de software,
especialmente em áreas como busca de duplicados, melhoria do relato e atribuição
e classificação automatizadas.
...
igual a 1). Todavia ao bem do nosso conhecimento não há técnicas para
classificação que não tenha influência da subjetividade.
...
partir deste levantamento foi obtido as principais funcionalidades existentes
