...
verificação de uma RM pode ser realizada pelos seguintes métodos: demonstração,
...
de \textit{50} ferramentas, fornecidas comercialmente ou em código
aberto\footnote{\url{https://en.wikipedia.org/wiki/Comparison_of_issue-tracking_systems}}.
Apesar das diversas opções disponíveis, ao bem do nosso conhecimento, são poucos
os estudos que avaliam sistematicamente as funcionalidades oferecidas por este
tipo de software. A nossa expectativa é que, a partir de um conjunto compartilhado
de funções e comportamentos, sejamos capazes de caracterizar as FGRMs, ao mesmo
tempo que possibilitaria avaliar a contribuição de novas funcionalidades
propostas na li\-te\-ra\-tu\-ra, conforme será apresentado no
...
Internet de algumas FGRMs de modo a compilar e agrupar as funcionalidades
...
esforço necessário para analisar a documentação de cada uma delas, optamos por
realizar este estudo com 6 ferramentas que foram escolhidas com a ajuda de
profissionais envolvidos em manutenção de software. A opinião dos profissionais
foi coletada através de um levantamento por questionário (survey) em que eles
selecionavam as FGRMs que julgavam mais representativas dentre aquelas que lhe
foi apresentada a partir de uma lista previamente definida.
...
por meio de um levantamento (survey). Acreditamos que o resultado obtido
permitirá uma melhor compreensão deste tipo de software tomando como base o
conjunto de funções que eles oferecem aos seus usuários. Em um segundo momento
também será possível propor novas funcionalidades ou melhorias das existentes
tendo em vista a possibilidade de determinar o conjunto mínimo de comportamentos
das FGRMs.
...
campo de texto em que era possível registrar outras FGRMs que ele entenda
...
diferentes nacionalidades foi criada uma versão em língua inglesa do formulário.
...
pontuação igual a 5. Após o cálculo de pontuação de cada ferramenta, ordenamos
...
Entre os materiais utilizados encontram-se manuais do usuário e de desenvolvedor
e notas de lançamento. Para cada uma das FGRMs optamos por analisar a última
versão estável do software a fim de analisarmos o que há de mais novo disponível
aos usuários. A documentação de algumas ferramentas, em especial aquelas que
adotam uma arquitetura cliente/servidor e necessitam de um certo grau de
administração, dividem as funcionalidades do software entre aquelas com foco no
usuário final e ad\-mi\-nis\-tra\-do\-res de sistema. Nestes casos, optamos por
coletar as funcionalidades cujo foco seja o usuário da FGRM, tendo em vista que
funcionalista com foco administradores não estão no escopo desta dissertação.
...
sistematizados por meio da técnica denominada \textit{Cartões de
    Classificação~-~Sorting Cards}. Trata-se de uma processo de elicitação de
conhecimento de baixo custo e com foco no usuário que é utilizada na área de
...
Existem três fases dentro do processo de Cartões de Classificação ($i$)
preparação, no qual os participantes ou o conteúdo dos cartões são selecionados;
($ii$) execução, em que os cartões são organizados em grupos significativos com
um título que seja capaz de descrever o grupo; ($iii$) análise, no qual os
cartões são sistematizados para formar hierarquias mais abstratas e que são
usadas para deduzir resultados. No processo tradicional de Cartões de
Classificação cada declaração realizada por um participante resulta na criação
de exatamente um único cartão~\cite{just2008towards}. Contudo, no nosso caso,
cada funcionalidade obtida mediante a inspeção da documentação foi mapeada em um
único cartão.
...
do qual tivemos 11 participantes. O restante é composto por 01 profissional que
se dedica a projetos de software livre e 01 estudante.
...
desenvolvimento de software em uma equipe de aproximadamente dez membros. Este
...
observado foi escolhido três softwares que conforme os critérios que adotamos
...
localização de duplicadas. Durante o processo de criação de uma RM, uma das
ferramentas possui a funcionalidade para detecção automatizada de duplicadas.
...
utilizando dispositivos móveis ou mediante formulários que podem ser criados em
...
Domínio Específico (Domain-Specific Language \@-\@ DSL em inglês) baseada em
SQL\@.
...
informações contidas na FGRM\@. As ferramentas analisadas possuem
funcionalidades para \textit{suportar múltiplas bases de dados}, como suporte
aos diferentes Sistemas de Gerenciamento de Banco de Dados disponíveis no
mercado. Além disso, a ferramenta Bugzilla oferece funcionalidade para validação
...
seu conteúdo, visualize e procure por alterações realizadas. As ferramentas
também possibilitam acesso à diferentes tipos de SCV, tais como Git, SVN,
Mercurial e etc.
...
funções para gerenciamento de tarefas e suporte à múltiplos projetos. Também é
possível personalizar o fluxo de trabalho adotado. Esta customização é realizada
...
tais como documentos de requisitos, código fonte, registros (logs) de teste e
assim por diante~\cite{cavalcanti2013bug}. Em alguns contextos, devido ao volume
de artefato gerados, é importante que a FGRM forneça suporte para armazenamento
e recuperação deste ativos do processo de desenvolvimento de um software. As
FGRMs possuem funcionalidades que interagem diretamente com a documentação de
software, geralmente no formato de Wikis. Além disso, algumas ferramentas
permitem uma melhor visualização de anexos incluídos na RM, como por exemplo
arquivos no formato CSV (comma-separated values).
...
classificação através do uso de etiquetas previamente definidas.
...
e/ou adaptação da interface para uma língua e cultura de um país. As FGRMs
possuem tradução para diversos idiomas e também possuem funcionalidades que
permitem à colaboradores criarem novas traduções.
...
das RMs que estão sendo analisadas. Neste contexto, as FGRMs oferecem
funcionalidades para visualizar a informação das RMs mediante quadro como
aqueles utilizados nas metodologias ágeis do tipo Kanban ou SCRUM\@. Existem
...
que ajudam aos gerentes de projetos na tomada de decisão.
...
pelas FGRMs evoluíram de simplesmente gerenciar as RMs para colaborar no
processo de desenvolvimento e manutenção do software. Entretanto, esta evolução
não é tão rápida quanto o necessário. As ferramentas analisadas apresentaram um
suporte bem estabelecido para atividades relativas à gestão da RM, como por
exemplo a criação de uma nova RM\@. Contudo, são bastantes escassas as funções
que minimizem os problemas que ocorrem quando as RMs são criadas. Por exemplo
não foi possível verificar funcionalidades nativas das RMs que ajudem ao
responsável pelo relato em fornecer as informações que serão necessária à
solução do que está sendo pedido. Uma outra facilidade ausente neste tipo de
ferramenta é a identificação de possíveis RMs duplicadas.
...
resultar em atrasos em situações em que se tem muitas RMs. Um exemplo deste
problema ocorre no processo de atribuição do desenvolvedor responsável por
solucionar determinada RM\@. Esta atividade fica sob a responsabilidade do
\textit{Agente de Triagem}, que deve realizar a escolha de forma manual já que
as FGRMs não são capazes de ``recomendar'' o desenvolvedor mais apto.
...
do relato e atribuição e classificação automatizadas das RMs. Algumas sugestões
de novas funcionalidades para as FGRMs serão discutidas no
...
apresentamos uma análise das funcionalidades das FGRMS realizado com o suporte
de um levantamento por questionário. A partir deste levantamento foi obtido as
principais funções existentes nas FGRMs. O conjunto comum de funcionalidades que
