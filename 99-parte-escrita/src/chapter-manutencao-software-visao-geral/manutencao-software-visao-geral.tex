...
%Modificação: qua fev 22 14:15:23 BRT 2017
...
\subsection{Modelo Conceitual das FGRM's}
...
temas recorrentes da definição de FGRM's encontradas nos artigos. Foram
...
\begin{description}
	\item[Projeto:] O Projeto reflete o contexto no qual uma determinada FGRM
		está inserida. Ele é composto pelos \textit{Componentes de Software} e
		demais \textit{Artefatos}. Os \textit{Componentes de Software}
		representam em conjunto o representa o sistema do qual as RM's estão
		vinculadas. Os \textit{Artefatos} são os demais itens de configuração
		que são utilizados ou produzidos no desenvolvimento do software tais
		como o código fonte, documentação, casos de teste e etc.
	\item[Repositório de RM:] Trata-se do banco de dados, manual ou automático,
		em que as RM's são armazenadas e gerenciadas. Cada item nesta base é
		representado por uma RM com as características discutidas na
		Seção~\ref{sec:requisicao_de_mudanca}.
	\item[Repositórios de Pessoas:] Representa a base de dados de usuários da
		FGRM. Nele são gerenciados os dados das pessoas envolvidas no projeto
		e de seus respectivos direitos de acesso às informações das RM's.
	\item[Fluxo de Trabalho:] O \textit{Fluxo de Trabalho} representa o conjunto
		...
		dele que é definido os diferente \textit{estados} que uma RM pode assumir desde
		de quando ela é redigida até o momento em que se define que foi
		solucionada. Este processo é realizado pelas \textit{Pessoas} envolvidas
		no \textit{Projeto} através dos diferente \textit{Papéis} desempenhados
		e suas respectivas \textit{Atividades}. Uma discussão mais aprofundada
		sobre os papéis desempenhados na manutenção de software está disponível
		na Seção~\ref{subsec:man_visao_geral_papeis_na_manutencao_de_software}.
		De maneira relacionada, os diferentes estados de um ciclo de vida de uma
		RM estão descritos na Seção~\ref{sub:fluxo_de_trabalho_requisicao_mudanca}.
\end{description}

\begin{figure}[htpb]
	\centering
	\includegraphics[width=0.8\linewidth]{./chapter-manutencao-software-visao-geral/img/diagrama-classe-conceitual-fgrm.pdf}
	\caption{Modelo conceitual de uma FGRM.}
