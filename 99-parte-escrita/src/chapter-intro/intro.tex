...
ocorrem no ambiente de execução e o atendimento a novos requisitos é solicitado.
O software depois de desenvolvido e entregue entra em uma fase convencionalmente
conhecida como Manutenção. A atividade de manutenção consiste em corrigir
defeitos descobertos, adaptar a novas demandas, aperfeiçoar e prevenir problemas
ainda não descobertos. Estas tarefas têm o apoio de ferramentas dentre elas a
que denominamos \texttt{Ferramentas de Gerenciamento de Requisições de Mudanças
    (FGRM)}. Esta dissertação apresenta o estudo de FGRMs feito no
