...
%Criação: Ter Out 11 19:03:45 BRT 2016 Modificação:
%seg dez 19 21:41:29 BRST 2016 
...
\paragraph{Visualização de RM} Os estudos classificados neste tópicos propõe
...
classificação previamente definido. Este esquema pode ser classificar em
...
como \textit{resolver um problema de software} ou \textit{implementar uma
	melhoria.} A diversa de categorias em determinado esquema de classificação
pode tornar complexo a tarefa, tendo em vista que em muitos casos não é fácil
determinar os limites entre os tipos~\cite{antoniol2008bug}. Por exemplo, a uma
classificação incorreta de um defeito como melhoria pode acarretar em atrasos no
projeto ou mesmo fazendo que uma RM receba menos atenção do que ela
mereça~\cite{cavalcanti2014challenges}.

\paragraph{Estimativa de Esforço da RM} A gestão do custo e esforço de um
...
geral três forma para medir este esforço~\cite{cavalcanti2014challenges}:
determinar o tempo para solucionar novas RM's; determinar os artefatos que são
...
complexidade produzir uma estimativa precisa por conta das diferentes atividades
envolvidas em conjunto aos diferente níveis de capacitação que o responsável
pela execução das tarefas pode ter~\cite{xia2015automatic}.  Apesar da inerente
imprecisão deste tipo de trabalho é importante salientar que estimar o tempo de
solução de uma RM é importante para o gerenciamento do projeto porque ajudar
alocar recurso de forma mais
...
\paragraph{Gerente de Requisição de	Mudança} O papel que representa esta
...
\paragraph{Líder da Manutenção} De forma similar ao Gerente de Requisição de
...
\section{Limitações e Ameaças à Validade} \label{sec:limitacoes_ameacas}
...
Este trabalho estende a classificação realizada por Cavalcanti tendo em vista
